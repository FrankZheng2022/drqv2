...
class BEEDrQV2Agent:
    def __init__(self, obs_shape, action_shape, device, lr, encoder_lr, feature_dim,
        ...
        self.v = VNetwork(self.encoder.repr_dim, feature_dim,
                             hidden_dim).to(device)
        ...
        parameters = itertools.chain(self.critic.parameters(),
                                     self.v.parameters(),
        )
        self.critic_opt = torch.optim.Adam(parameters, lr=lr)
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        with torch.no_grad():
            ...
            target_V_exploit = self.v(next_obs)
            target_V = self.lamb * target_V_exploit + (1-self.lamb)*target_V_explore
        ...
        ### compute loss for V
        with torch.no_grad():
            Q1, Q2  = self.critic_target(obs, action, self.act_tok)
            Q = torch.min(Q1, Q2)
        V = self.v(obs)
        ...
        v_loss = (vf_weight * (vf_err ** 2)).mean()
