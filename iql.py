...
class IQLAgent:
    def __init__(self, obs_shape, action_shape, device, lr, encoder_lr, feature_dim,
        ...
        self.V = V(self.encoder.repr_dim, feature_dim, hidden_dim).to(device)
    ...
    def update_v(self, obs, action, reward, discount, next_obs, critic_target):
        metrics = dict()
        
        Q1, Q2 = critic_target(obs, action)
        q = torch.min(Q1, Q2)
        v = self.V(obs)
        value_loss = torch.mean(utils.expectile_loss(q-v, self.expectile))
        
        # optimize encoder and v
        ...
        if self.use_tb:
            metrics['v_loss'] = value_loss.item()
        return metrics
    ...
    def update_actor(self, obs, b_act, step):
        metrics = dict()      
    ...
    def update(self, replay_iter, step):
        ...
        # update V
        metrics.update(
            self.update_v(obs_en.detach(), action, reward, discount, next_obs_en, self.critic_target))
        
        #obs_en = self.encoder(self.aug(obs.float()))
        # update critic
        metrics.update(
